...
func TestSubject_Updates(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		if err := m.Updates(Subject{SubjName: "UpdatedName", SubjType: "UpdatedT"}); err != nil { // SubjType was to long for MariaDB
			t.Fatal(err)
		} else {
			assert.Equal(t, "UpdatedName", m.SubjName)
			assert.Equal(t, "UpdatedT", m.SubjType) // SubjType was to long for MariaDB
		}
	})
