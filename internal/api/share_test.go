...
func TestGetShares(t *testing.T) {
	...
	t.Run("valid token and share", func(t *testing.T) {
		app, router, _ := NewApiTest()
		Shares(router)
		r := PerformRequest(app, "GET", "/api/v1/4jxf3jfn2k/as6sg6bxpogaaba7") // Used to panic because the templates weren't loaded
	...
	t.Run("valid token", func(t *testing.T) {
		app, router, _ := NewApiTest()
		Shares(router)
		r := PerformRequest(app, "GET", "/api/v1/4jxf3jfn2k") // Used to panic because the templates weren't loaded
		assert.Equal(t, http.StatusOK, r.Code)
	})
}
