...
test.meta("testID", "account-001").meta({ type: "short", mode: "auth" })(
  ...
  async (t) => {
    ...
    await t.click(settings.accountTab).click(account.MFAAction).expect(account.setup.visible).ok().click(account.close);
    await page.logout();
    await page.login("jane", "photoprism");
    await t.expect(page.passcodeInput.visible).notOk();
    await page.logout();
  }
);

test.meta("testID", "account-002").meta({ type: "short", mode: "auth" })("Core: Create App Password", async (t) => {
  await page.login("admin", "photoprism");
  await menu.openPage("settings");
  await t
    .click(settings.accountTab)
    .click(account.appsAndDevicesAction)
    .expect(Selector("td").withText("M@#y-app-passw&^[]/'ord").visible)
    .notOk()
  ...
  await t
    ...
    .expect(Selector("td").withText("M@#y-app-passw&^[]/'ord").visible)
    .ok()
    .expect(Selector('tr[data-name="M@#y-app-passw&^[]/\'ord"] td').nth(2).innerText)
    .contains("–")
    .click(account.close);
  ...
  await t
    .click(settings.accountTab)
    .expect(account.changePasswordAction.hasAttribute("disabled"))
    .ok()
    .click(account.appsAndDevicesAction)
    .expect(Selector("td").withText("M@#y-app-passw)(&^{}[];/'ord").visible)
    .notOk()
...
test.meta("testID", "account-003").meta({ type: "short", mode: "auth" })(
  ...
  async (t) => {
    ...
    await menu.openPage("settings");
    await t.click(settings.accountTab).click(account.appsAndDevicesAction);
    await t
...
test.meta("testID", "account-005").meta({ type: "short", mode: "auth" })("Core: Delete App Password", async (t) => {
  ...
  await t
    ...
    .click(account.confirm)
    .expect(Selector("td").withText("admin-full").visible)
    .notOk()
    .click(account.close);
...
test.meta("testID", "account-006").meta({ type: "short", mode: "auth" })(
  ...
  async (t) => {
    ...
    await t.click(settings.accountTab).click(account.MFAAction);
    await t
